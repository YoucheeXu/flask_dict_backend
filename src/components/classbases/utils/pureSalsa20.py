...
"""
    ...
    Sample usage:
        ...
        s20 = Salsa20(key, iv)
        dataout = s20.encrypt_bytes(datain)   # same for decrypt
    ...
    Perhaps giving each instance its own rounds variable, which
    ...
    ----------
      ...
      p3.1   Took timing variability out of add32() and rot32().
             ...
             in encrypt_bytes(), modify a byte array instead of appending.
...
class Salsa20():
    ...
    '''
    def __init__(self, key: bytes | None = None, iv: bytes | None = None, rounds: int = 20):
        self._last_chunk64: bool = True
        self._IVbitlen: int = 64             # must be 64 bits
        self.ctx: list[int] = [ 0 ] * 16
        if key:
            self.set_key(key)
        if iv:
            self.iv: bytes = iv
    ...
    # setNonce = set_iv            # support an alternate name
    ...
    def set_rounds(self, rounds: int, testing: bool = False):
        assert testing or rounds in [8, 12, 20] # rounds must be 8, 12, 20
        self.rounds = rounds

    def encrypt_bytes(self, data: bytes) -> bytes:
        assert isinstance(data, bytes) # data must be byte string
        assert self._last_chunk64 # previous chunk not multiple of 64 bytes
        lendata = len(data)
        munged = bytearray(lendata)
        for i in range(0, lendata, 64):
            h: bytes = salsa20_wordtobyte(self.ctx, self.rounds, is_check_rounds=False)
        ...
        self._last_chunk64 = not lendata % 64
        return bytes(munged)

    # decryptBytes = encrypt_bytes # encrypt and decrypt use same function


def salsa20_wordtobyte(inval: list[int] | tuple[int, int], rounds: int = 20, is_check_rounds: bool = True):
    """ Do rounds Salsa20 rounds on a copy of 
            inval: list or tuple of 16 ints treated as little-endian unsigneds.
        Returns a 64-byte string.
        """

    assert type(inval) in (list, tuple)  and  len(inval) == 16
    assert not(is_check_rounds) or (rounds in [ 8, 12, 20 ])
    ...
    for i in range(len(inval)):
        x[i] = PLUS(x[i], inval[i])
    return little16_i32.pack(*x)


def trunc32(w: int) -> int:
...
def rot32(w: int, left: int):
    """ Rotate 32-bit word left by left or right by -nLeft
        without creating a Python long.
        Timing depends on nLeft but not on w.
    """
    left &= 31  # which makes left >= 0
    if left == 0:
        return w

    # Note: now 1 <= left <= 31.
    #     RRRsLLLLLL   There are left RRR's, (31-left) LLLLLL's,
